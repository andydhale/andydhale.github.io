...
export function generateStaticParams() {
    ...
    return pageCounts.map((page) => ({ pageNum: [page] }));
  }


export default function BlogIndex({ params }: { params: { pageNum: string  } }) {
    if (!params.pageNum) {
        params.pageNum = "1";
    }

    const posts = getPosts(+params.pageNum ?? 1, 3);

    const prevPage = +params.pageNum - 1;
